@php
    ...
    $description = $description ?? 'Laporan masuk untuk ditinjau.';
    $mode = $mode ?? 'supervisor'; // supervisor | manager
        ...
        <div class="flex overflow-x-auto border-b border-gray-100 scrollbar-none">
            @foreach ($tabs as $key => $tabDef)
                @php
                    ...
                    $count = $counts[$key] ?? 0;
                @endphp
                <a href="{{ route($tabRouteName, ['tab' => $key]) }}"
                        ...
                        @foreach ($reports as $report)
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-5 py-3.5">
                                    ...
                                    @elseif ($report->approval_status === 'returned')
                                ...
                                <td class="px-5 py-3.5">
                                    @if ($mode === 'supervisor' && $report->status === 'returned_to_supervisor')
                                        @php $mngrApproval = $report->approvals->firstWhere('step', 3); @endphp
                                        <div class="flex flex-col items-center gap-1">
                                            <a href="{{ route($reviewRouteName, $report->id) }}"
                                    ...
                                    @else
                                        <a href="{{ route($reviewRouteName, $report->id) }}"
                                           class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg {{ $reviewButtonClass }} text-xs font-medium transition-colors">
                                            Tinjau
                                            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
