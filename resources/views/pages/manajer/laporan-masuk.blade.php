...
<div class="space-y-5">
    ...
    <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5">Laporan dari Supervisor yang dikirimkan kepada Anda untuk ditinjau.</p>
    </div>
        ...
        <div class="flex overflow-x-auto border-b border-gray-100 scrollbar-none">
            @foreach ($tabs as $key => $tabDef)
                @php
                    ...
                    $count    = $counts[$key] ?? 0;
                @endphp
                <a href="{{ route('manajer.laporan-masuk', ['tab' => $key]) }}"
                   class="flex items-center gap-2 px-4 py-3.5 text-sm font-medium whitespace-nowrap border-b-2 transition-colors
                          {{ $isActive ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-200' }}">
                    {{ $tabDef['label'] }}
                    @if ($count > 0)
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @foreach ($reports as $report)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-5 py-3.5">
                                    <a href="{{ route('manajer.laporan.show', $report->id) }}"
