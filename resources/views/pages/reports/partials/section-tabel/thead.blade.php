<thead>
    ...
    <tr class="bg-sky-50 text-gray-600 border-b border-sky-100">
        ...
        @php
            ...
            $msStartLocked = (bool) ($ms0TimeLockedByOther['start_time'] ?? false);
            $msEndLocked = (bool) ($ms0TimeLockedByOther['end_time'] ?? false);
        @endphp
        ...
        @php
            ...
            $timeLocks = is_array($timeLockedByOther[$col] ?? null) ? $timeLockedByOther[$col] : [];
            $timeStartLocked = (bool) ($timeLocks['start_time'] ?? false);
            $timeEndLocked = (bool) ($timeLocks['end_time'] ?? false);
        @endphp
        <th class="px-2 py-1.5 text-center font-semibold border-r border-sky-100 whitespace-nowrap"
            ...
            <div class="space-y-0.5 mt-1">
                ...
                @php
                    ...
                    $abLocks = $timeLockedByOther[$col][$ab] ?? [];
                    $abStartLocked = (bool) ($abLocks['start_time'] ?? false);
                    $abEndLocked = (bool) ($abLocks['end_time'] ?? false);
                @endphp
                <div class="flex items-center justify-center gap-0.5">
