                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-5 py-3.5 text-center">
                                    ...
                                    @elseif ($item->status === 'returned')
                                        @php
                                            $retApproval = $item->approvals
                                                ->where('status', 'returned')
                                                ->sortByDesc('updated_at')
                                                ->first();
                                            $isReturnTarget = (string) ($retApproval?->returned_to_user_id ?? '') === (string) auth()->id();
                                        @endphp
                                        <div class="flex flex-col items-center gap-1">
                                            <a href="{{ route('reports.fill', $item) }}"
                                               class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors {{ $isReturnTarget ? 'bg-orange-500 text-white hover:bg-orange-600' : 'bg-gray-50 text-gray-600 hover:bg-gray-100 border border-gray-100' }}">
                                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    @if ($isReturnTarget)
                                                ...
                                                {{ $isReturnTarget ? 'Revisi' : 'Lihat' }}
                                            </a>
                                            @if ($isReturnTarget && $retApproval?->notes)
                                    ...
                                    @elseif ($item->status === 'pending')
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            <button type="button"
                                                    data-url="{{ route('reports.fill', $item) }}"
                                                    data-product="{{ $item->product_name }}"
                                                    data-batch="{{ $item->batch_number }}"
                                                    onclick="confirmMulaiFromButton(this)"
...
<script>
function confirmMulaiFromButton(button) {
    if (!button) return;
    const url = button.dataset.url || '#';
    const product = button.dataset.product || '';
    const batch = button.dataset.batch || '';
    const mode = button.dataset.mode || undefined;
    confirmMulai(url, product, batch, mode);
}
